	    ...
	    Garden.randomrgba = function (rmin, rmax, gmin, gmax, bmin, bmax, a) {
			var limit = 5;
			// retry a few times to avoid near-gray colors; always return a valid rgba
			...
			// fallback: use midpoints if all retries were too similar
			return Garden.rgba(
				Math.round((rmin + rmax) / 2),
				Math.round((gmin + gmax) / 2),
				Math.round((bmin + bmax) / 2),
				a
			);
	    };
